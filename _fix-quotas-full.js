...
(async () => {
  ...
  const summary = { total: shops.length, createdWallet: 0, updatedWallet: 0, updatedLegacy: 0 };
  ...
  for (const shop of shops) {
    ...
    if (!wallet) {
      ...
      wallet = await prisma.quotaWallet.findUnique({ where: { shopId: shop.id } });
    }
    ...
    const updateWallet = {};
    if (wallet.weeklyLiveBaseLimit !== baseLive) updateWallet.weeklyLiveBaseLimit = baseLive;
    if (wallet.reelDailyLimit !== baseReel) updateWallet.reelDailyLimit = baseReel;
    if (wallet.weeklyLiveWeekKey !== weekKey) updateWallet.weeklyLiveWeekKey = weekKey;
    if (wallet.reelDailyDateKey !== dayKey) updateWallet.reelDailyDateKey = dayKey;
    if (wallet.weeklyLiveUsed !== nextWeeklyUsed) updateWallet.weeklyLiveUsed = nextWeeklyUsed;
    if (wallet.reelDailyUsed !== nextDailyUsed) updateWallet.reelDailyUsed = nextDailyUsed;
    ...
    if (Object.keys(updateWallet).length > 0) {
      ...
      wallet = { ...wallet, ...updateWallet };
    }

    const baseRemaining = Math.max(0, baseLive - nextWeeklyUsed);
    const reelRemaining = Math.max(0, baseReel - nextDailyUsed);
    const nextStreamQuota = baseRemaining + (wallet.liveExtraBalance || 0);
    const nextReelQuota = reelRemaining + (wallet.reelExtraBalance || 0);
    ...
    if (index % 50 === 0) {
      console.log(`Progreso: ${index}/${shops.length}`);
    }
  }
